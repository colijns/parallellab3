...
// 线程数：测试时用 -DGEN_THREADS=2 / 4 / 8 改
#ifndef GEN_THREADS
#define GEN_THREADS 4
#endif

// 阈值：小于该 value 数量时直接串行，大于等于时才并行
...
void PriorityQueue::Generate_openmp(PT pt)
{
    ...
    // 每个线程写自己的局部结果数组，避免多个线程同时写 guesses
    // 相比 vector<string> temp(total)，这种写法不会提前默认构造大量 string
    const int num_threads = GEN_THREADS;
    vector<vector<string>> local_results(num_threads);

    for (int t = 0; t < num_threads; t++)
    {
        int start = t * total / num_threads;
        int end = (t + 1) * total / num_threads;
        local_results[t].reserve(end - start);
    }
...
#pragma omp parallel num_threads(GEN_THREADS)
#endif
    {
        ...
        vector<string> &local = local_results[tid];
...
static void *GeneratePthreadWorker(void *arg)
{
    PthreadGenerateArgs *args = (PthreadGenerateArgs *)arg;

    args->local_result->reserve(args->end - args->start);

    for (int i = args->start; i < args->end; i++)
    {
        args->local_result->emplace_back(args->prefix + (*(args->values))[i]);
    }
...
void PriorityQueue::Generate_pthread(PT pt)
{
    ...
    // 小任务直接串行，避免 pthread_create / pthread_join 开销
    if (total < GEN_THRESHOLD)
    {
        guesses.reserve(guesses.size() + total);
    ...
    PthreadGenerateArgs args[GEN_THREADS];
    vector<vector<string>> local_results(GEN_THREADS);
    ...
    for (int t = 0; t < GEN_THREADS; t++)
    {
        ...
        args[t].local_result = &local_results[t];

        pthread_create(&threads[t], nullptr, GeneratePthreadWorker, &args[t]);
    }
